...
export default function InvestmentsPage() {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 24 }}>Investments</h1>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} style={{ padding: 24, textAlign: "center", color: "#94A3B8" }}>No investments found</td></tr>
              ) : (
                investments.map((inv) => (
                  <tr key={inv.id} style={{ borderBottom: "1px solid #2A3444", cursor: "pointer" }}>
                    <td style={{ padding: "12px 16px" }}>
                      <Link href={`/investments/${inv.id}`} style={{ textDecoration: "none" }}>
                        ...
                        <div style={{ fontSize: 12, color: "#94A3B8" }}>{inv.investor?.user?.email}</div>
                      </Link>
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <Link href={`/deals/${inv.deal?.id}`} style={{ fontSize: 14, color: "#C8A24D", textDecoration: "none" }}>
                        {inv.deal?.name || "—"}
                      </Link>
                    </td>
                    ...
                    <td style={{ padding: "12px 16px", fontSize: 14, color: "#FFFFFF", fontWeight: 500 }}>
