...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, { bg: string; text: string }> = {
    ...
    VERIFIED: { bg: "rgba(16, 185, 129, 0.1)", text: "#10B981" },
...
export default function InvestorsPage() {
  ...
  const fetchInvestors = async (page = 1) => {
    ...
    try {
      const res = await authFetch(`${API_URL}/api/admin/investors?${params}`);
  ...
  return (
    <div>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 24, color: "#0A1F44" }}>Investors</h1>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <form onSubmit={handleSearch} style={{ display: "flex", gap: 8 }}>
          ...
          />
          <button type="submit" style={{ padding: "8px 16px", backgroundColor: "#0A1F44", color: "#FFFFFF", border: "none", borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
            Search
          </button>
        </form>
      </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} style={{ padding: 24, textAlign: "center", color: "#6B7280" }}>No investors found</td></tr>
              ) : (
                investors.map((inv) => (
                  <tr key={inv.id} style={{ borderBottom: "1px solid #E5E7EB", cursor: "pointer" }} onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "#F9FAFB")} onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}>
                    <td style={{ padding: "12px 16px", fontSize: 14 }}>
                      <Link href={`/investors/${inv.id}`} style={{ color: "#111827", textDecoration: "none" }}>
                        {inv.fullName || `${inv.user?.firstName || ""} ${inv.user?.lastName || ""}`}
                      </Link>
                    </td>
                    <td style={{ padding: "12px 16px", fontSize: 14, color: "#6B7280" }}>{inv.user?.email}</td>
                    <td style={{ padding: "12px 16px", fontSize: 13, color: "#6B7280" }}>{inv.countryOfResidence || "—"}</td>
                    <td style={{ padding: "12px 16px", fontSize: 13, color: "#6B7280" }}>{inv.investorType || "—"}</td>
                    <td style={{ padding: "12px 16px", fontSize: 13, color: "#6B7280" }}>{inv.accreditationBasis || "—"}</td>
                    <td style={{ padding: "12px 16px" }}><StatusBadge status={inv.verificationStatus || "PENDING"} /></td>
                    <td style={{ padding: "12px 16px", fontSize: 13, color: "#6B7280" }}>{new Date(inv.createdAt).toLocaleDateString()}</td>
                  </tr>
                ))
              )}
            </tbody>
