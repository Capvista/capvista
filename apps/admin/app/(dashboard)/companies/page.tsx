...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, { bg: string; text: string }> = {
    ...
    INFO_REQUESTED: { bg: "rgba(59, 130, 246, 0.1)", text: "#3B82F6" },
  };
...
export default function CompaniesPage() {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 24, color: "#0A1F44" }}>Companies</h1>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} style={{ padding: 24, textAlign: "center", color: "#6B7280" }}>No companies found</td></tr>
              ) : (
                companies.map((c) => (
                  <tr key={c.id} style={{ borderBottom: "1px solid #E5E7EB", cursor: "pointer" }} onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "#F9FAFB")} onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}>
                    <td style={{ padding: "12px 16px" }}>
                      <Link href={`/companies/${c.id}`} style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none", color: "#111827" }}>
                        {c.logoUrl ? (
                          <img src={c.logoUrl} alt="" style={{ width: 32, height: 32, borderRadius: 6, objectFit: "cover" }} />
                        ) : (
                          <div style={{ width: 32, height: 32, borderRadius: 6, backgroundColor: "#F3F4F6", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#6B7280" }}>
                    ...
                    <td style={{ padding: "12px 16px", fontSize: 14 }}>
                      <div style={{ color: "#111827" }}>{c.owner?.user?.firstName} {c.owner?.user?.lastName}</div>
